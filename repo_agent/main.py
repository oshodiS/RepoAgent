...
@click.command()
@click.option('-ch', '--chunk-size', default=250, help='Specify the chunk size')
@click.option('-cho', '--chunk-overlap', default=30, help='Specify the chunk overlap')
def chat_with_repo(chunk_size, chunk_overlap):
    """Automatic Q&A for Documentation Explanation"""
    markdown_folder = setting.project.target_repo / setting.project.markdown_docs_name
    hierarchy_file = setting.project.target_repo / setting.project.hierarchy_name / "project_hierarchy.json"
...
@click.option('-ch', '--chunk-size', default=250, help='Specify the chunk size')
@click.option('-cho', '--chunk-overlap', default=30, help='Specify the chunk overlap')
def show_chunk(chunk_size, chunk_overlap):
